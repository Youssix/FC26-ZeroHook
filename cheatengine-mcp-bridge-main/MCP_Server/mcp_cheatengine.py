...
@mcp.tool()
def evaluate_lua(code: str, async_mode: bool = False, timeout: int = 10000) -> str:
    """Execute arbitrary Lua code in Cheat Engine. Set async_mode=True when code uses createThread/executeCodeEx/autoAssemble+execution to avoid GUI deadlock."""
    params = {"code": code}
    if async_mode:
        params["async"] = True
        params["timeout"] = timeout
    return format_result(ce_client.send_command("evaluate_lua", params))
