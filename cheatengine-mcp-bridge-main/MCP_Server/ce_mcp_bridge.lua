...
local function cmd_evaluate_lua(params)
    ...
    -- async mode: run in a separate thread to avoid deadlock when code
    -- uses createThread/waitfor/executeCodeEx (which need the GUI thread)
    if params.async then
        local done = false
        local asyncResult = nil
        local asyncError = nil

        local t = createThread(function(thread)
            local ok2, res = pcall(fn)
            if ok2 then
                asyncResult = tostring(res)
            else
                asyncError = tostring(res)
            end
            done = true
        end)

        -- Wait up to 10 seconds for completion
        local timeout = params.timeout or 10000
        local waited = 0
        ...
        if not done then
            ...
            return { success = false, error = "Async execution timed out after " .. timeout .. "ms" }
        end

        if asyncError then
            return { success = false, error = "Runtime error: " .. asyncError }
        end
